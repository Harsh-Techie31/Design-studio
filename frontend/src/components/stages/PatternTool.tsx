      ...
      <Section label="B. Tech Pack Reference">
        {selectedTechPack ? (
          <div className="relative">
            ...
            <button onClick={() => setTpPickerOpen(true)} className="mt-1 text-[11px] text-accent hover:text-accent-soft">Switch tech pack</button>
          </div>
        ) : (
          <button onClick={() => setTpPickerOpen(true)} className="flex w-full flex-col items-center gap-2 rounded-lg border-2 border-dashed border-line bg-ink-soft p-6 transition-colors hover:border-accent/30">
          ...
          {Object.entries(bodyMeasurements).map(([field, value]) => (
            <div key={field} className="flex items-center gap-2 rounded-lg border border-line bg-ink-soft px-2.5 py-1.5">
              ...
              <input
                ...
                className={`w-14 rounded border border-line bg-ink px-1.5 py-0.5 text-right font-mono text-[11px] focus:border-accent/60 focus:outline-none ${
                  editedMeasurements.has(field) ? "text-bone" : "text-accent"
                }`}
              />
      ...
      <div className="mt-auto flex flex-col gap-3 border-t border-line pt-4">
            ...
            {[1, 2, 3, 4].map((n) => (
              <button
                ...
                className={`rounded px-3 py-1 text-sm transition-all ${
                  numOutputs === n ? "bg-accent text-ink font-semibold" : "bg-ink-soft text-muted hover:text-bone-dim"
                }`}
              >
                {n}
              </button>
        ...
        <button
          ...
          className={`flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-bold uppercase tracking-wider transition-all ${
            !selectedTechPack ? "cursor-not-allowed bg-line text-muted" : "bg-accent text-ink hover:bg-accent-soft"
          }`}
        >
          {isGenerating ? (
            <>
  ...
  return (
    <div className="mb-6 border-t border-line pt-5 first:border-0 first:pt-0">
      <div className="mb-3 flex items-center gap-2">
        <label className="text-[13px] font-semibold uppercase tracking-widest text-accent">{label}</label>
