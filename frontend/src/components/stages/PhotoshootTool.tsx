    ...
    <aside className="flex h-full w-full flex-col overflow-y-auto bg-surface p-6 lg:w-[480px]">
      ...
      <Section label="B. Mockup Reference" required>
        {vizImage ? (
          <div className="relative">
            ...
            <button onClick={() => setVizPickerOpen(true)} className="mt-1 text-[11px] text-accent hover:text-accent-soft">
              Switch mockup
            </button>
          </div>
        ) : (
        ...
        <button
          ...
          className={`flex w-full items-center justify-between rounded-lg border px-3.5 py-2.5 transition-all ${
            !moodboardReady
              ...
              : "border-line bg-ink-soft hover:border-accent/30"
          }`}
        >
          ...
          <span
            className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${
              moodboardReady && moodboardInfluence ? "bg-accent" : "bg-line"
            }`}
          >
            <span
      ...
      <Section label="H. Pose">
        ...
        <input
          ...
          className="mt-2 w-full rounded-lg border border-line bg-ink-soft px-3 py-2 text-sm text-bone placeholder:text-muted focus:border-accent/60 focus:outline-none"
        />
      </Section>
      ...
      <div className="mt-auto flex flex-col gap-3 border-t border-line pt-4">
        ...
        <button
          ...
          className={`flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-bold uppercase tracking-wider transition-all ${
            !vizImage ? "cursor-not-allowed bg-line text-muted" : "bg-accent text-ink hover:bg-accent-soft"
          }`}
        >
          {isGenerating ? (
            <>
