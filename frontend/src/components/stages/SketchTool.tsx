  ...
  return (
    <aside className="flex h-full w-full flex-col overflow-y-auto bg-surface p-6 lg:w-[480px]">
      ...
      <button
        ...
        className="mb-5 flex w-full items-center justify-center gap-2 rounded-lg border border-accent/50 bg-transparent py-2.5 text-sm text-accent transition-colors hover:bg-accent/10 disabled:opacity-50"
      >
        {isUploading ? (
          <>
      ...
      <Section label="1. Category" hint="From garment">
        <div className="inline-flex items-center gap-2 rounded-lg border border-accent/30 bg-accent/10 px-3 py-1.5">
          <i className="ti ti-shirt text-sm text-accent" />
          <span className="text-sm font-medium text-accent">{CATEGORY_LABELS[category]}</span>
        </div>
      </Section>
            ...
            {hasMoodboard && (
              <div className="mt-3 rounded-lg border border-line bg-ink-soft p-3">
                <div className="mb-1 flex justify-between">
                  ...
                  <span className="text-[12px] font-bold text-accent">{moodInfluence}%</span>
                </div>
                <input
                  type="range"
                  min="0"
