...
def _build_framing_logic(category_code: str) -> str:
...
def _build_visualization_prompt(
    ...
    model_desc = MODEL_DESCRIPTIONS.get(model_avatar, MODEL_DESCRIPTIONS["Model A"])
    framing_logic = _build_framing_logic(category_code)
    ...
    return (
        ...
        f"3. **Framing & Camera:** {framing_logic}\n"
...
# ─── Fallback placeholder (mirrors the lightweight PIL fallback used by other stages) ───


def _build_placeholder_image(category_display: str, model_avatar: str) -> bytes:
    img = Image.new("RGB", (1200, 750), (244, 242, 238))
    ...
    try:
        ...
        font_small = font
    draw.text((430, 340), "[3D Visualization]", fill=(160, 160, 160), font=font)
    draw.text(
        (330, 380),
...
async def generate_visualization(
    ...
    if render_img:
        render_bytes = await _fetch_image_bytes(render_img.url)
    else:
        render_bytes = await _fetch_image_bytes(body.render_image_url)
    ...
    category_display = CATEGORY_DISPLAY_NAMES.get(category_code.upper(), category_code.title())
    ...
    if not generated_images:
        ...
        placeholder_bytes = _build_placeholder_image(category_display, body.model_avatar)
