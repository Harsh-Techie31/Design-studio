...
async def generate_visualization(
    ...
    # ─── Create NodeRun ───
    iteration = await _next_iteration(garment_id, NodeKey.VISUALIZATION)
    logger.info("[VIS] New iteration=%d", iteration)
    ...
    for idx, img_data in enumerate(generated_images):
        ...
        while True:
            ...
            try:
                ...
                count += 1
                img_code = f"{code}_{count:02d}"
                logger.warning("[VIS] Duplicate image_code, retrying with %s", img_code)
                try:
